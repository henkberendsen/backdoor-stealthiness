#!/usr/bin/env python3
"""
Spectral Signatures: Detecting Backdoor Attacks on Deep Neural Networks

Implementation of Spectral Signatures defense for backdoor detection.
The defense works by analyzing the singular value decomposition (SVD) of
activation patterns to identify poisoned training samples as outliers.
...
Usage:
    python defenses/ss.py --attack badnet --model resnet18 --dataset cifar10 --poison_rate 0.05 --exp_num 1
"""
...
class SpectralSignaturesDetector:
    """
    Spectral Signatures detector for backdoor detection.
    """
    
    def __init__(self, model, model_name, device, num_classes, percentile=85, random_seed=42):
        """
        Initialize the Spectral Signatures detector.
        ...
        Args:
            ...
            percentile: Percentile threshold for outlier detection (default: 85)
    ...
    def detect(self, train_loader, train_labels):
        """
        Detect poisoned samples in the training set using spectral signatures.
        ...
        print(f"  Performing per-class spectral analysis...")
        ...
        # Perform spectral analysis per class
        for target_class in range(self.num_classes):
            ...
            full_mean = np.mean(class_features, axis=0, keepdims=True)
            centered_features = class_features - full_mean
            
            # Perform SVD decomposition
            try:
                u, s, v = np.linalg.svd(centered_features, full_matrices=False)
                print(f"      Top 5 singular values: {s[:5]}")
            except np.linalg.LinAlgError as e:
                print(f"      Warning: SVD failed: {e}, skipping class")
                continue
            
            # Use top singular vector
            top_eigenvector = v[0:1]  # shape: (1, feature_dim)
            
            # Calculate correlation scores
            # corrs shape: (1, num_samples_in_class)
            corrs = np.matmul(top_eigenvector, np.transpose(class_features))
            
            # Calculate norm of correlations (outlier score for each sample)
            scores = np.linalg.norm(corrs, axis=0)  # shape: (num_samples_in_class,)
            
            # Determine threshold based on percentile
            threshold_score = np.percentile(scores, self.percentile)
            
            # Find outliers (samples with scores above threshold)
            outlier_indices_in_class = np.where(scores > threshold_score)[0]
            
            # Map back to original dataset indices
            outlier_indices = class_indices[outlier_indices_in_class]
            
            print(f" Threshold: {threshold_score:.4f}, Outliers: {len(outlier_indices)}")
            
            suspicious_indices.extend(outlier_indices.tolist())
...
def load_attack_data(attack_name, dataset_name, model_arch, poison_rate, device):
    ...
    if bd_train is None:
        ...
        raise ValueError(
            ...
            f"Note: Spectral Signatures requires training data. "
            f"This attack may not be compatible with SS defense."
        )
...
def run_spectral_signatures_defense(attack_name, model_arch, dataset_name, poison_rate,
                                     percentile=85, batch_size=128, 
                                     random_seed=42, exp_num=None):
    """
    Run Spectral Signatures defense on a backdoor attack.
    ...
    """
    print("=" * 70)
    print("Spectral Signatures Backdoor Defense")
    print("=" * 70)
    print(f"Attack: {attack_name}")
    print(f"Model: {model_arch}")
    print(f"Dataset: {dataset_name}")
    print(f"Poison Rate: {poison_rate}")
    print(f"SS Parameters: percentile={percentile}")
    print("=" * 70)
    ...
    # Initialize detector
    print(f"\nInitializing Spectral Signatures detector...")
    ...
    csv_path = RESULTS_DIR / "spectral_signatures.csv"
    
    results = {
        'exp_num': exp_num,
        'defense_name': 'SpectralSignatures',
        'attack': attack_name,
        'model': model_arch,
        'dataset': dataset_name,
        'poison_rate': poison_rate,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Spectral Signatures Defense Against Backdoor Attacks',
    ...
    try:
        print("Starting Spectral Signatures Defense")
        run_spectral_signatures_defense(
